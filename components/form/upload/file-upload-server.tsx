...
export function FileUploadServer({
  ...
  const uploadFile = async (file: File, options: ServerUploadOptions): Promise<ServerUploadResponse> => {
    try {
      ...
      // Handle unwrapped response from RTK Query interceptor
      if (result && (result.publicUrl || result.fullPath)) {
        return {
          success: true,
          data: result
        };
  ...
  const uploadFiles = async () => {
    ...
    try {
      const uploadPromises = pendingFiles.map(async (file) => {
        ...
        // Use originalFile if available, as spreading a File object loses its properties
        ...
        setFiles(prev => prev.map(f => 
          f.id === file.id 
            ? { 
                ...
                progress: 100,
                response,
                error: response.success ? undefined : (response.error || 'Upload failed')
              } 
            : f
        ));
