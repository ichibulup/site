...
export default function OrderPage() {
  ...
  const ordersData: OrderListItem[] = Array.isArray(orders) ? (orders as OrderListItem[]) : []
  ...
  const averageOrderValue =
    stats?.averageOrderValue ?? (ordersData.length > 0 ? totalRevenue / ordersData.length : 0)
  ...
  const renderOrderStatusBadge = (status?: string) => {
    ...
    return <Badge variant={getOrderStatusVariant(normalized)}>{label}</Badge>
  }
    ...
    <div className="flex flex-1 flex-col gap-4 p-4 pt-0">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{stats?.totalOrders || ordersData.length}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {stats?.preparingOrders || ordersData.filter((o) => o.status === "preparing" || o.status === "confirmed").length}
            </div>
            <p className="text-xs text-muted-foreground">
              Cần theo dõi
            </p>
